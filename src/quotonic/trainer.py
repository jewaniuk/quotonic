...
class Trainer:
    ...
    Attributes:
        ...
        mesh (Mesh): object containing methods that allow linear layers (i.e. rectangular Mach-Zehnder interferometer meshes) to be encoded and decoded, passed up from child class, otherwise defaults to a 4-mode mesh
    """
    ...
    def __init__(self, num_trials: int, num_epochs: int, print_every: int = 10, mesh: Optional[Mesh] = None) -> None:
        ...
        Args:
            ...
            mesh: object containing methods that allow linear layers (i.e. rectangular Mach-Zehnder interferometer meshes) to be encoded and decoded
        """
    ...
    def initialize_params(self, L: int) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
        ...
        Returns:
            A tuple containing arrays of the $\\phi$ ($L\\times m(m-1) / 2$), $\\theta$ ($L\\times m(m-1) / 2$), and $\\delta$ ($L\\times m$) phase shifts for each component of the QPNN
        """

        # generate a random unitary from the Haar measure for each layer, and perform Clements decomposition to extract the corresponding random phases
        m = self.mesh.m
        phi, theta, delta = (
...
class IdealTrainer(Trainer):
    ...
    Attributes:
        ...
        num_epochs (int): number of training epochs to run
        print_every (int): specifies how often results should be printed, in terms of epochs
        mesh (Mesh): object containing methods that allow linear layers (i.e. rectangular Mach-Zehnder interferometer meshes) to be encoded and decoded, taken from IdealQPNN instance
        qpnn (IdealQPNN): object containing methods to construct the transfer function enacted by a QPNN, and compute the network fidelity
    ...
    def __init__(
        ...
        Args:
            qpnn: object containing methods to construct the transfer function enacted by a QPNN, and compute the network fidelity
            num_trials: number of training trials to run
            num_epochs: number of training epochs to run
            print_every: specifies how often results should be printed, in terms of epochs
    ...
    def cost(self, phi: jnp.ndarray, theta: jnp.ndarray, delta: jnp.ndarray) -> DTypeLike:
        """Evaluate the cost function that is minimized during training.

        Args:
            phi: $L\\times m(m-1)/2$ phase shifts, $\\phi$, where the ith row contains those for each MZI in the ith layer of the QPNN being trained
            theta: $L\\times m(m-1)/2$ phase shifts, $\\theta$, where the ith row contains those for each MZI in the ith layer of the QPNN being trained
            delta: $L\\times m$ phase shifts, $\\delta$, where the ith row contains those for each mode at the output of the mesh in the ith layer of the QPNN being trained
    ...
    def update(
        self, phi: jnp.ndarray, theta: jnp.ndarray, delta: jnp.ndarray, optstate: optax.OptState
    ) -> Tuple[DTypeLike, Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray], optax.OptState]:
        ...
        Returns:
            A tuple including the current value of the cost function, another tuple of the current parameters ($\\phi$, $\\theta$, $\\delta$), and the current state of the optimizer
        """
        # calculate cost function and its gradient with respect to the 0th, 1st, 2nd parameters, which are phi, theta, delta respectively
...
class ImperfectTrainer(Trainer):
    ...
    Attributes:
        ...
        num_epochs (int): number of training epochs to run
        print_every (int): specifies how often results should be printed, in terms of epochs
        mesh (Mesh): object containing methods that allow linear layers (i.e. rectangular Mach-Zehnder interferometer meshes) to be encoded and decoded, taken from ImperfectQPNN instance
        qpnn (ImperfectQPNN): object containing methods to construct the transfer function enacted by a QPNN, and compute the network performance measures
        sched (optax.Schedule): the exponential decay scheduler used during optimization
        opt (optax.GradientTransformation): the adam optimizer used during optimization
    """
    ...
    def __init__(
        ...
        Args:
            qpnn: object containing methods to construct the transfer function enacted by a QPNN, and compute the network performance measures
            num_trials: number of training trials to run
            num_epochs: number of training epochs to run
            print_every: specifies how often results should be printed, in terms of epochs
            sched0: initial value of the exponential decay scheduler used during optimization
...
class MinimalTrainer(Trainer):
    """Class for training minimal QPNNs based on single-site Kerr-like nonlinearities that may be turned on/off and varied in strength.

    Attributes:
        num_trials (int): number of training trials to run
        num_epochs (int): number of training epochs to run
        print_every (int): specifies how often results should be printed, in terms of epochs
        mesh (Mesh): object containing methods that allow linear layers (i.e. rectangular Mach-Zehnder interferometer meshes) to be encoded and decoded, taken from MinimalQPNN instance
        qpnn (MinimalQPNN): object containing methods to construct the transfer function enacted by a QPNN, and compute the network performance measures
        sched (optax.Schedule): the exponential decay scheduler used during optimization
        opt (optax.GradientTransformation): the adam optimizer used during optimization
    """
    ...
    def __init__(
        ...
        """Initialization of a Minimal Trainer instance.
        ...
        # store the provided properties of the Minimal Trainer
        self.qpnn = qpnn
    ...
    def train(self) -> dict:
        ...
        results = {
            ...
            "theta": np.empty((self.num_trials, self.qpnn.L, self.qpnn.m * (self.qpnn.m - 1) // 2), dtype=float),
            "delta": np.empty((self.num_trials, self.qpnn.L, self.qpnn.m), dtype=float),
            "varphi": np.empty((self.num_trials, self.qpnn.L - 1, self.qpnn.m), dtype=float),
            "ell_mzi": np.empty((self.num_trials, self.qpnn.L, self.qpnn.m, self.qpnn.m), dtype=float),
            "ell_ps": np.empty((self.num_trials, self.qpnn.L, self.qpnn.m), dtype=float),
            "t_dc": np.empty((self.num_trials, self.qpnn.L, 2, self.qpnn.m * (self.qpnn.m - 1) // 2), dtype=float),
        }
        ...
        for trial in range(self.num_trials):
            ...
            # refresh the imperfection model for the qpnn if not the first trial
            if trial > 0:
                self.qpnn = MinimalQPNN(
                    self.qpnn.n,
                    self.qpnn.m,
                    self.qpnn.L,
                    varphi=self.qpnn.varphi,
                    burnout_map=self.qpnn.burnout_map,
                    ell_mzi=self.qpnn.ell_mzi,
                    ell_ps=self.qpnn.ell_ps,
                    t_dc=self.qpnn.t_dc,
                    training_set=self.qpnn.training_set,
...
class JitterTrainer(Trainer):
    """Class for training QPNNs based on single-site Kerr-like nonlinearities that operate on partially-distinguishable photons with finite wavepackets.

    Attributes:
        num_trials (int): number of training trials to run
        num_epochs (int): number of training epochs to run
        print_every (int): specifies how often results should be printed, in terms of epochs
        mesh (Mesh): object containing methods that allow linear layers (i.e. rectangular Mach-Zehnder interferometer meshes) to be encoded and decoded, taken from JitterQPNN instance
        qpnn (JitterQPNN): object containing methods to construct the transfer function enacted by a QPNN, and compute the network performance measures
        sched (optax.Schedule): the exponential decay scheduler used during optimization
        opt (optax.GradientTransformation): the adam optimizer used during optimization
    """
    ...
    def __init__(
        ...
        """Initialization of a Jitter Trainer instance.

        Args:
            qpnn: object containing methods to construct the transfer function enacted by a QPNN, and compute the network fidelities
            num_trials: number of training trials to run
            num_epochs: number of training epochs to run
            print_every: specifies how often results should be printed, in terms of epochs
            sched0: initial value of the exponential decay scheduler used during optimization
        ...
        # store the provided properties of the Jitter Trainer
        self.qpnn = qpnn
    ...
    def cost(self, phi: jnp.ndarray, theta: jnp.ndarray, delta: jnp.ndarray) -> DTypeLike:
        """Evaluate the cost function that is minimized during training, as well as the unconditional wavepacket fidelity.
        ...
        Returns:
            Cost (i.e. network error) of the QPNN, followed by the unconditional wavepacket fidelity
        """
        Fuw, Fub = self.qpnn.calc_unc_fidelities(phi, theta, delta)
        return 1 - Fub, Fuw
    ...
    def update(
        ...
        Returns:
            A tuple including the current value of the cost function, the current unconditional wavepacket fidelity, another tuple of the current parameters ($\\phi$, $\\theta$, $\\delta$), and the current state of the optimizer
        """
        # calculate cost function and its gradient with respect to the 0th, 1st, 2nd parameters, which are phi, theta, delta respectively
        (C, Fuw), grads = value_and_grad(self.cost, argnums=(0, 1, 2), has_aux=True)(phi, theta, delta)

        # calculate updates to the parameters and the state of the optimizer from the gradients, then apply them
        updates, optstate = self.opt.update(grads, optstate)
        phi, theta, delta = optax.apply_updates((phi, theta, delta), updates)

        return C, Fuw, (phi, theta, delta), optstate
    ...
    def train(self) -> dict:
        ...
        # prepare the results dictionary
        results = {
            "Fub": np.empty((self.num_trials, self.num_epochs), dtype=float),
            "Fuw": np.empty((self.num_trials, self.num_epochs), dtype=float),
            "Fcb": np.empty((self.num_trials,), dtype=float),
            "Fcw": np.empty((self.num_trials,), dtype=float),
            "rate": np.empty((self.num_trials,), dtype=float),
            "phi0": np.empty((self.num_trials, self.qpnn.L, self.qpnn.m * (self.qpnn.m - 1) // 2), dtype=float),
            "phi": np.empty((self.num_trials, self.qpnn.L, self.qpnn.m * (self.qpnn.m - 1) // 2), dtype=float),
            "theta0": np.empty((self.num_trials, self.qpnn.L, self.qpnn.m * (self.qpnn.m - 1) // 2), dtype=float),
            "theta": np.empty((self.num_trials, self.qpnn.L, self.qpnn.m * (self.qpnn.m - 1) // 2), dtype=float),
            "delta0": np.empty((self.num_trials, self.qpnn.L, self.qpnn.m), dtype=float),
            "delta": np.empty((self.num_trials, self.qpnn.L, self.qpnn.m), dtype=float),
        }
        ...
        for trial in range(self.num_trials):
            ...
            Theta = Theta0
            optstate = initial_optstate
            Fub = np.zeros(self.num_epochs, dtype=float)
            Fuw = np.zeros(self.num_epochs, dtype=float)
            for epoch in range(self.num_epochs):
                C, Fuw[epoch], Theta, optstate = self.update(*Theta, optstate)
                Fub[epoch] = 1 - C
...
class TreeTrainer(Trainer):
    ...
    Attributes:
        ...
        num_epochs (int): number of training epochs to run
        print_every (int): specifies how often results should be printed, in terms of epochs
        mesh (Mesh): object containing methods that allow linear layers (i.e. rectangular Mach-Zehnder interferometer meshes) to be encoded and decoded, taken from TreeQPNN instance
        qpnn (TreeQPNN): object containing methods to construct the transfer function enacted by a QPNN, and compute the network performance measures
        sched (optax.Schedule): the exponential decay scheduler used during optimization
        opt (optax.GradientTransformation): the adam optimizer used during optimization
    """
    ...
    def train(self) -> dict:
        ...
        results = {
            "Fu": np.empty((self.num_trials, self.num_epochs), dtype=float),
            "Fur": np.empty((self.num_trials,), dtype=float),
            "Fuu": np.empty((self.num_trials,), dtype=float),
            "Fc": np.empty((self.num_trials,), dtype=float),
            "rate": np.empty((self.num_trials,), dtype=float),
            "phi": np.empty((self.num_trials, self.qpnn.L, self.qpnn.m * (self.qpnn.m - 1) // 2), dtype=float),
            "theta": np.empty((self.num_trials, self.qpnn.L, self.qpnn.m * (self.qpnn.m - 1) // 2), dtype=float),
        ...
        for trial in range(self.num_trials):
            ...
            if trial > 0:
                self.qpnn = TreeQPNN(
                    ...
                    t_dc=self.qpnn.t_dc,
                    training_set=self.qpnn.training_set,
                    training_set_route=self.qpnn.training_set_route,
                )
            ...
            # compute performance measures of the trained QPNN
            _, Fur, Fuu, Fc, rate = self.qpnn.calc_performance_measures(*Theta)
            print(f"COMPLETE! \t Cost: {C:.4e} \t Fur: {Fur:.4g} \t Fuu: {Fuu:.4g} \t Fc: {Fc:.4g} \t Rate: {rate:.4g}")

            # store the results from this trial
            results["Fu"][trial] = Fu
            results["Fur"][trial] = Fur
...
class TreeTrainerExtended(Trainer):
    """Class for training imperfect QPNNs based on three-level system photon subtraction/addition nonlinearities that power a tree-type photonic cluster state generation protocol.
    ...
    Attributes:
        ...
        print_every (int): specifies how often results should be printed, in terms of epochs
        mesh (Mesh): object containing methods that allow linear layers (i.e. rectangular Mach-Zehnder interferometer meshes) to be encoded and decoded, taken from TreeQPNN instance
        qpnn (TreeQPNNExtended): object containing methods to construct the transfer function enacted by a QPNN, and compute the network performance measures
        sched (optax.Schedule): the exponential decay scheduler used during optimization
        opt (optax.GradientTransformation): the adam optimizer used during optimization
    """
